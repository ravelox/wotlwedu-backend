...
### `GET /ai/election/:electionId/recommendations`

```sh
ELECTION_ID="election_000"
curl -sS "$API/ai/election/$ELECTION_ID/recommendations" -H "Authorization: Bearer $TOKEN"
```

### `GET /ai/election/:electionId/summary`

```sh
ELECTION_ID="election_000"
curl -sS "$API/ai/election/$ELECTION_ID/summary" -H "Authorization: Bearer $TOKEN"
```

### `GET /ai/election/:electionId/suggest-participants`

```sh
ELECTION_ID="election_000"
curl -sS "$API/ai/election/$ELECTION_ID/suggest-participants" -H "Authorization: Bearer $TOKEN"
```

### `POST /ai/list/suggest-items`

```sh
curl -sS "$API/ai/list/suggest-items" \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"Healthy lunch options"}'
```

### `POST /ai/item/categorize`

```sh
curl -sS "$API/ai/item/categorize" \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text":"Pizza with mushrooms"}'
```

### `POST /ai/moderate`

```sh
curl -sS "$API/ai/moderate" \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text":"Example text"}'
...
### `GET /ai/image/:imageId/describe`

```sh
IMAGE_ID="image_000"
curl -sS "$API/ai/image/$IMAGE_ID/describe" -H "Authorization: Bearer $TOKEN"
```

### `GET /ai/notification/digest`

```sh
curl -sS "$API/ai/notification/digest" -H "Authorization: Bearer $TOKEN"
```

### `GET /ai/preferences/defaults`

```sh
curl -sS "$API/ai/preferences/defaults" -H "Authorization: Bearer $TOKEN"
```

### `POST /ai/assistant/query`

```sh
curl -sS "$API/ai/assistant/query" \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"query":"Suggest three items for a new election list"}'
```
